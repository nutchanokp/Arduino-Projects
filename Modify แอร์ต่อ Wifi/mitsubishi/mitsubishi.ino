...
void hpSettingsChanged() {
  ...
  doc["power"]       = currentSettings.power;
  doc["mode"]        = currentSettings.mode;
  doc["temperature"] = currentSettings.temperature;
...
void hpStatusChanged(heatpumpStatus currentStatus) {
  ...
  DynamicJsonDocument rootInfo(2);
  ...
  DynamicJsonDocument rootTimers(5);
...
void hpPacketDebug(byte* packet, unsigned int length, const char* packetDirection) {
  if (_debugMode) {
    ...
    // const size_t bufferSize = JSON_OBJECT_SIZE(6);
    // DynamicJsonDocument root(bufferSize);
    DynamicJsonDocument root(6); // ใช้ heap แทน stack
